...
yum -y install gcc make gcc-c++ kernel-devel-`uname -r` zlib-devel openssl-devel readline-devel sqlite-devel wget
...
yum -y install automake autoconf gettext pcre-devel
yum -y install perl perl-ExtUtils-MakeMaker
